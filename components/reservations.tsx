...
export function CreateForm({
  ...
  const [fields, setFields] = useState<ReservationFields>(() => {
    ...
    return {
      ...
      maxPlayers: String(reservation?.maxPlayers ?? 5),
      ...
      <form
        ...
        onSubmit={(event) => {
          ...
          const parsed = createSchema.safeParse(
            creationInputFromFields(fields),
          );
          if (!parsed.success) {
          ...
          startTransition(async () => {
            try {
              ...
              const key = reservation ? undefined : submissionKey(parsed.data);
              if (!reservation) setSubmission(readSubmission());
